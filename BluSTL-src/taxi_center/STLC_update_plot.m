...
if isempty(Sys.h)
    ...
    %subplot(nb_plots,2,[2, 4, 6, 8, 10]);
    subplot(nb_plots,2,[2, 4, 6]);
    hold on; grid on;
    ...
    if ~isempty(Sys.objective_alw_ev)
        ...
        %end
        plot(getPoly([0.5, 1, 3, 3.5]),ops);
        plot(getPoly([0.5, 1, 0.5, 1]),ops);
        plot(getPoly([1.5, 2, 1, 1.5]),ops);
        plot(getPoly([4.5, 5, 3, 3.5]),ops);
        plot(getPoly([3.5, 4, 2, 2.5]),ops);
    ...
    %axis([0 20 0 20]);
    % Change 
    % Sys.h.Xpast(5) = plot(Sys.system_data.X(1,1:end-1), Sys.system_data.X(3,1:end-1),system_style{:});
    % Sys.h.Xmodel(5) = plot(Sys.model_data.X(1,:),Sys.model_data.X(3,:), model_style{:});
    Sys.h.Xpast(3) = plot(Sys.system_data.X(1,1:end-1), Sys.system_data.X(3,1:end-1),system_style{:});
    Sys.h.Xmodel(3) = plot(Sys.model_data.X(1,:),Sys.model_data.X(3,:), model_style{:});
    %ylabel(Sys.xlabel{3});
    ylabel(Sys.xlabel{2});
    xlabel(Sys.xlabel{1});
    ...
    set(Sys.h.Xpast(3), 'Xdata', Sys.system_data.X(1,1:end-1), 'Ydata',Sys.system_data.X(3,1:end-1));
    set(Sys.h.Xmodel(3), 'Xdata', Sys.model_data.X(1,:),'Ydata', Sys.model_data.X(3,:));
