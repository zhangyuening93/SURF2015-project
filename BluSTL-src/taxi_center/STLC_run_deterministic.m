function [Sys, flag, params] = STLC_run_deterministic(Sys, controller, publisher)
...
while (time_d(end)+ts< time(end))
    ...
    function update_hist_data()
        ...
        %% Modify for the ros program.
        my_msg = rosmessage(publisher);
        my_msg.X = Xpred(1,:);
        % my_msg.Y = Xpred(3,:);
        my_msg.Y = Xpred(2,:);
        my_msg.ID = int16(time_new/ts +1);
        send(publisher, my_msg);
    ...
    function update_controller_data()
        ...
        [Wn_new, Wref, ID] = Sys.getW();
        while ID < int16(time_new/ts)%+ 1 - 1
            %str = sprintf('%d < %d now.',ID,int16(time_new/Sys.ts));
            %display(str)
            %display('Run_deterministic is waiting for W to update.')
            pause(0.2);
            % CHANGE!
            [Wn_new, Wref, ID] = Sys.getW();
            %display(Sys.Wref);
            %display(Sys.current_w_ID);
