classdef STLC_lti_2 < handle
    ...
    methods
        function Sys = STLC_lti_2(varargin)
        ...
        function obj = get_objective(Sys, X, Y, U,W, rho,wr)
            switch nargin
                ...
                case 7
                    if (Sys.objFuncVersion == 2)
                        obj = -wr*sum(rho);
                        display(strcat('The objective function is -',num2str(wr),'phi.'));
                    elseif (Sys.objFuncVersion == 1)
                        obj = sum(sum(abs(U)))-wr*sum(rho);
                        display(strcat('The objective function is mini_U-',num2str(wr),'phi.'));
                    end
                   % obj = sum(sum(abs(U)));
                   % obj = sum(sum(abs(U)))-wr*sum(rho);
            end
        ...
        function [Sys, flag, params] = run_deterministic(Sys, controller)
            [Sys, flag, params] = STLC_run_deterministic(Sys, controller);
        end
